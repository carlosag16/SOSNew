            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  ...
                  $id = DBRead("agd_agendamentos","agd_id","WHERE agd_concluido = 1".$add_ser_id.";");
                  //pega número de matrícula do usuário
                  $matricula = DBRead("agd_agendamentos","agd_matricula","WHERE agd_concluido = 1".$add_ser_id.";");
                  // pega o nome do usuário
                  $nome = DBRead("agd_agendamentos","agd_nome","WHERE agd_concluido = 1".$add_ser_id.";");
                  // pega o email do usuário
                  $email = DBRead("agd_agendamentos","agd_email","WHERE agd_concluido = 1".$add_ser_id.";");
                  // pega o telefone do usuário
                  $telefone = DBRead("agd_agendamentos","agd_telefone","WHERE agd_concluido = 1".$add_ser_id.";");
                  // pega a data marcada do agendamento
                  $datas = DBRead("agd_agendamentos","agd_data","WHERE agd_concluido = 1".$add_ser_id.";");
                  // pega a hora marcada do agendamento
                  $horas = DBRead("agd_agendamentos","hor_id","WHERE agd_concluido = 1".$add_ser_id.";");
                  // pega o curos do usuário
                  $curso = DBRead("agd_agendamentos","agd_curso_programa","WHERE agd_concluido = 1".$add_ser_id.";");
                  // pega a data em que foi aberta a solicitação de agendamento
                  $criadas = DBRead("agd_agendamentos","agd_criada","WHERE agd_concluido = 1".$add_ser_id.";");
                  // pega resultadop se compareceu ou não compareceu
                  $nao_compareceu = DBRead("agd_agendamentos","agd_nao_compareceu","WHERE agd_concluido = 1".$add_ser_id.";");
                  // para pegar estado de cancelado
                  $cancelou = DBRead("agd_agendamentos","agd_cancelado","WHERE agd_concluido = 1".$add_ser_id.";");
                  //para pegar o servidor que ira atender a solicitação (so o adm pode ver isso)
                  $_ids = DBRead("agd_agendamentos","ser_id","WHERE agd_concluido = 1".$add_ser_id.";");
                  $ser_nome = array();
                  foreach ($_ids as $_id) {
                  ...
                  for ($i=0; $i < count($nome); $i++) {
                    ...
                    */
                    if($nao_compareceu[$i]==1){
                      ...
                      echo '<td><center><span class="badge danger" title="Solicitante não compareceu">Ausente</span><center></td>';
                    }elseif ($cancelou[$i]==1){
                      echo '<td><center><span class="badge secondary">Cancelado</span><center></td>';
                    }else{
                      echo '<td><center><span class="badge success" title="Solicitante compareceu">Presente</span><center><pan></td>';
                    }
                    echo '</tr>';
                  }
